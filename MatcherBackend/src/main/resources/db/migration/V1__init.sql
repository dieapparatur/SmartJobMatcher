    CREATE TABLE country (
      ...
      name VARCHAR(255) NOT NULL, -- 'English country name',
      full_name VARCHAR(255) NOT NULL, -- 'Full English country name',
      iso3 CHAR(3) NOT NULL, -- 'Three-letter country code (ISO 3166-1 alpha-3)',
      number CHAR(3) NOT NULL, -- 'Three-digit country number (ISO 3166-1 numeric)',
...
CREATE TABLE candidate (
    ...
    preferred_employment_type TEXT NOT NULL, --full time, part time, etc.
    preferred_work_location CHAR(2) REFERENCES country(code),
    highest_degree TEXT,
...
CREATE TABLE job_posting (
    ...
    location CHAR(2) REFERENCES country(code),
    employment_type TEXT NOT NULL,
    related_company BIGINT REFERENCES company(id) NOT NULL
);


CREATE TABLE interested_job_candidates (
    id BIGSERIAL PRIMARY KEY,
    job_posting_id BIGINT REFERENCES job_posting(id) NOT NULL,
    candidate_id BIGINT REFERENCES candidate(id) NOT NULL,
    status TEXT NOT NULL, -- will switch depending if the candidate passed (necessary?), liked, or then got matched
    -- which will then be an entry into matched_candidates, and hide the candidate (only then visible in matched list)
    CONSTRAINT candidate_x_posting UNIQUE (job_posting_id, candidate_id)
);


CREATE TABLE matched_candidates ( -- will make a chat or something possible
    id BIGSERIAL PRIMARY KEY,
    job_posting_id BIGINT REFERENCES job_posting(id) NOT NULL,
    candidate_id BIGINT REFERENCES candidate(id) NOT NULL,
    CONSTRAINT candidate_x_posting_x_company UNIQUE (job_posting_id, candidate_id)
);